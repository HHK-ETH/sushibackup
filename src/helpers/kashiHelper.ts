...
export async function fetchKashiPairsData(
  ...
  return (
    await Promise.all(
      pairsData.map(async (pair, index): Promise<IKashiPairData> => {
        ...
        const apr: number = parseFloat(
          formatUnits(BigNumber.from(pair.accrueInfo.interestPerSecond).mul(BigNumber.from(3600 * 24 * 365 * 100)))
        );
        const totalAsset: number = parseFloat(
          formatUnits(
            BigNumber.from(pair.totalBorrow.elastic).add(BigNumber.from(pair.totalAsset.elastic)),
            asset.decimals
          )
        );
        const totalBorrow: number = parseFloat(formatUnits(BigNumber.from(pair.totalBorrow.elastic), asset.decimals));
        const bentoContract: Contract = new Contract(
          PRODUCTS[PRODUCT_IDS.BENTOBOX].networks[chainId],
          PRODUCTS[PRODUCT_IDS.BENTOBOX].ABI,
          web3Provider.getSigner()
        );
        ...
        const userShares = pair.totalAsset.base.gt(0)
          ? pair.userAssetFraction.mul(allShare).div(pair.totalAsset.base)
          : 0;
        const userAsset = parseFloat(
          formatUnits(await bentoContract.toAmount(asset.address, userShares, true), asset.decimals)
        );
        return {
